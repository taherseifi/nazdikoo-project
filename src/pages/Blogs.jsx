  ...
  const seo = useMemo(() => {
    return buildSeoPayload({
      ...
      fallback: {
        title: 'وبلاگ',
        description:
          'مقاله‌ها و نکات کاربردی نزدیکو درباره خدمات، انتخاب بهتر و معرفی کسب‌وکارها.',
          ...
          {!loading && !error && (
            <div className="grid grid-cols-1 gap-6 md:grid-cols-2 xl:grid-cols-3">
              {blogs.map((blog) => (
                <article
                  key={blog.id}
                  className="overflow-hidden rounded-[28px] bg-white shadow-lg transition hover:-translate-y-1 hover:shadow-md border-slate-300 border"
                >
                  ...
                  <div className="p-6">
                    <Link to={`/blogs/${blog.slug}`}>
                      <h2 className="line-clamp-2 text-2xl font-bold text-slate-800">
                        {blog.title}
                      </h2>
                    </Link>
